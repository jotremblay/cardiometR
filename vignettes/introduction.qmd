...
# Preferred: one-call pipeline (mirrors the Shiny Results tab)
analysis <- analyze_cpet(data)
print(analysis@peaks)

# Or step by step when you need intermediate objects
validation <- validate(data)
averaged <- average(data, method = "rolling", window = 30)
peaks <- find_peaks(averaged, averaging = 30)
thresholds <- detect_thresholds(averaged, methods = c("v_slope", "ve_vo2"))
...
- Peaks and point VT estimates use **exercise breaths only** (rest/recovery excluded).
- VT confidence reflects **method agreement** within about 10% VO2, not only how many methods fired.
- Sport normative z-scores often use an **estimated SD** from published ranges; treat approximate percentiles carefully. Sport strata are largely age-blind today.
- Economy / substrate claims require a **steady-state** stage by default.
